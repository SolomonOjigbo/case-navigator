...
export type NotificationKind = "reply_to_topic" | "reply_after_you";
...
export async function listNotifications(limit = 50): Promise<NotificationItem[]> {
  const { data, error } = await supabase
    .from("community_notifications")
    .select("id,actor_id,kind,topic_id,comment_id,created_at,read_at")
  ...
  const [actors, { data: topics }, { data: comments }] = await Promise.all([
  ...
  return rows.map((r) => ({
    ...
    excerpt: r.comment_id ? (bodyById.get(r.comment_id) ?? null) : null,
  }));
}
