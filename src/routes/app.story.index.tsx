...
import { ChevronRight, Check, ShieldCheck, FileUp } from "lucide-react";
...
function StoryOverview() {
  ...
  return (
    <div className="reading-column py-2 sm:py-4">
      ...
      <div role="note" className="note-reassure mb-4 flex items-start gap-2.5 text-sm">
        <ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-primary" aria-hidden="true" />
        <p className="m-0">{t("story.overview.control_notice")}</p>
      </div>

      {/* For people who arrive with the whole thing already written — a common
          case, since many have told it before. */}
      <Link
        to="/app/story/import"
        className="surface-card-interactive mb-6 flex items-center gap-3 p-4 no-underline"
      >
        <span
          aria-hidden="true"
        ...
        <span className="min-w-0 flex-1 text-[0.9375rem] font-medium text-foreground">
          {t("import.open_import")}
        </span>
